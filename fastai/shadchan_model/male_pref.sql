DROP VIEW IF EXISTS males CASCADE;
DROP VIEW IF EXISTS female_approved_approved CASCADE;
DROP VIEW IF EXISTS female_approved_full CASCADE;
DROP VIEW IF EXISTS male_pref CASCADE;
DROP VIEW IF EXISTS total_males CASCADE;
...
SELECT DISTINCT male_id
FROM matches
WHERE ms = 'Closed';

CREATE VIEW female_approved AS
SELECT m.male_id, m.female_id
FROM matches m
JOIN males f ON m.male_id = f.male_id
WHERE m.ms = 'Closed'
ORDER BY m.male_id;

CREATE VIEW female_approved_full AS
SELECT ma.male_id, mv.* 
FROM member_values mv
JOIN female_approved ma ON mv.id = ma.female_id;
CREATE VIEW male_pref AS
SELECT
    male_id,
...
FROM female_approved_full
GROUP BY male_id;

CREATE VIEW total_males AS
    SELECT *
    FROM male_pref
    JOIN member_values
    ON male_pref.male_id = member_values.id;
SELECT *
FROM total_males;